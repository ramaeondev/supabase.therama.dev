...
// ramu@Ramas-Mac-mini supabase.therama.dev % for day in {1..25}; do
//   d=$(printf "2025-04-%02d" $day)
//   curl -X POST "https://gyyhnbzekafnvxflhlni.functions.supabase.co/s3-billing-metrics?date=$d"
// done
// {"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}{"success":true,"count":16}%                                                                                                                            
...
async function getBucketMetrics(bucket: string, _region: string, creds: any, start: Date, end: Date) {
...
serve(async (req) => {
  // Parse date param (YYYY-MM-DD), default to previous day
  let targetDay: Date;
  if (req) {
    try {
  ...
  for (const { name, region } of buckets) {
    console.log(`[metrics] Fetching metrics for bucket: ${name}, region: ${region}, date: ${start.toISOString().slice(0, 10)}`);
    const metrics = await getBucketMetrics(name, region, creds, start, end);
    data.push({
      bucket: name,
      region,
      ...metrics,
      collected_at: start.toISOString(),
    });
  }
